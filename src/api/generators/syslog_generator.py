...
class SyslogEvent:
    ...
    def to_rfc5424(self) -> str:
        ...
        return (f"<{self.priority}>1 {timestamp_str} {self.hostname} "
                f"{self.tag} - - {structured_data} {self.message}")
...
class SyslogGenerator:
    ...
    def _generate_sample_users(self) -> List[str]:
        ...
        common_users = [
            ...
            "administrator", "sa", "oracle", "postgres", "mysql"
        ]
    ...
    def generate_event(self, threat_type: ThreatType = None, 
                      severity: SyslogSeverity = None) -> SyslogEvent:
    ...
    async def send_event(self, event: SyslogEvent, sock: socket.socket = None) -> bool:
        ...
        try:
            ...
            self.metrics.events_by_severity[severity_name] = \
                self.metrics.events_by_severity.get(severity_name, 0) + 1
    ...
    async def run_simulation(self, 
                           ...
                           threat_mix: Dict[ThreatType, float] = None) -> SyslogMetrics:
        ...
        logger.info(f"Simulation completed. Sent: {self.metrics.events_sent}, "
